...
test_dataset = tf.data.Dataset.list_files('./data/TCIRRP/test1k/*.jpg')
...
input_model_path = "1k-20-epochs.keras"
output_model_path = "1k-40-v2-epochs.keras"
...
for name, dataset in [("Training set", train_dataset), ("Testing set", test_dataset)]:
    ...
    # 3) Plota
    plt.figure(figsize=(8, num_tests * 2))
    plt.suptitle(name, fontsize=16)
    for i in range(num_tests):
        # Coluna 0: pré-processada
        ax = plt.subplot(num_tests, 4, i*4 + 1)
        plt.imshow(255-x_input[i].numpy(), cmap="gray")
        ax.set_title("Input")
        ax.axis("off")

        # Coluna 1: original de entrada
        ax = plt.subplot(num_tests, 4, i*4 + 2)
        plt.imshow(x_input[i].numpy(), cmap="gray")
        ax.set_title("Reversed")
        ax.axis("off")

        # Coluna 2: reconstruída
        ax = plt.subplot(num_tests, 4, i*4 + 3)
        plt.imshow(decoded_imgs[i], cmap="gray")
        ax.set_title("Preddiction")
        ax.axis("off")

        # Coluna 3: imagem “real” alvo (se for diferente da entrada)
        ax = plt.subplot(num_tests, 4, i*4 + 4)
